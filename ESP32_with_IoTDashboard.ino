 ...
 *
 */
#define API_KEY "AIzaSyD2UqvYI2sXlhFG7bfLwEKuW0MH2JIIkkI"

/* 3. Define the user Email and password that already registerd or added in your project */
#define USER_EMAIL "xxxxx@xxxx.com"
#define USER_PASSWORD "xxxxx"

/* 4. If work with RTDB, define the RTDB URL */
#define DATABASE_URL "https://test-xxxxx-default-rtdb.firebaseio.com" //<databaseName>.firebaseio.com or <databaseName>.<region>.firebasedatabase.app
...
void loop()
{
    ...
    if (millis() - dataMillis > 10000 && Firebase.ready())
    {
        ...
        String base_path = user_id+"/Things/";
        
        //------------------------------------------------------------------------
        //Construct the topic path:
        String room1_temp_topic = "rm_1/temp";
        String rm_1_temp_path = base_path + room1_temp_topic;
        
        // Read "rm_1/temp" value:
        float room1_temp_setpoint = 0;
        if (Firebase.getString(fbdo, rm_1_temp_path + "/Value")) { 
              room1_temp_setpoint = fbdo.stringData().toFloat();
              Serial.println("room#1 temperature: "+fbdo.stringData());   
        }else Serial.println(fbdo.errorReason());

//        // Write in "rm_1/temp" feedback:
//        if (Firebase.setString(fbdo, rm_1_temp_path + "/Feedback", room1_temp_setpoint + 1)){
//              Serial.println(room1_temp_setpoint + 1);
//        }else Serial.println(fbdo.errorReason());
//        
        //------------------------------------------------------------------------
        //Construct the topic path:
        String room1_light_topic = "rm_1/light";
        String rm_1_light_path = base_path + room1_light_topic;
        
        // Read "rm_1/light" value:
        String room1_light_status;
        if (Firebase.getString(fbdo, rm_1_light_path + "/Value")) { 
              room1_light_status = fbdo.stringData()== "true"? "On" : "Off";
              Serial.println("room#1 light: "+room1_light_status);   
        }else Serial.println(fbdo.errorReason());

        //------------------------------------------------------------------------
        //Construct the topic path:
        String room1_curtn_topic = "rm_1/curtn";
        String rm_1_curtn_path = base_path + room1_curtn_topic;
        
        // Read "rm_1/curtn" value:
        String room1_curtain_status;
        if (Firebase.getString(fbdo, rm_1_curtn_path + "/Value")) { 
              room1_curtain_status = fbdo.stringData()== "true"? "Open" : "Close";
              Serial.println("room#1 curtain: "+room1_curtain_status);   
        }else Serial.println(fbdo.errorReason());
